...
自动导演、创作中枢和卷章规划最早面向长篇网文生产设计，曾把默认章数、最小目标章数和“前 30 章承诺”写成固定长篇尺度。这会让 3 章、5 章这类短篇项目被拉长，也会让提示词在短篇里要求不存在的固定章节号。

## Decision

系统必须把章节规模视为用户目标，而不是长篇默认值。用户明确输入 3 章、5 章等短篇目标时，规划链路应按短篇闭环生成紧凑结构。
...
- 自动导演目标章节数最小支持到 3 章，允许 3 章、5 章短篇。
...
- 如果 3 章项目在蓝图阶段失败，优先检查 arc 或 chapter shell 的 `min()` 约束是否重新变成多阶段长篇假设。
