...
- 异步队列必须恢复任务所属用户上下文后再执行模型调用或外部服务调用；图像生成、写法提取、RAG 索引、拆书生成、章节流水线和导演命令 worker 不能在无用户上下文中读取模型、图片模型、RAG 或写法引擎偏好。
...
4. 新增异步任务处理器时，如果任务表有 `userId`，执行模型调用前必须用任务 `userId` 恢复 auth context；如果任务表没有 `userId`，必须能从父任务或父资源解析 owner。
