...
- 异步队列必须恢复任务或资源所属用户上下文后再执行模型调用或外部服务调用；图像生成、写法提取、RAG 索引、拆书生成、章节流水线、导演命令 worker、自动导演小说绑定流水线、章节后台资产同步、兑现账本同步、侧效应 worker、漫画事实抽取、短剧批量任务和漫画批量任务不能在无用户上下文中读取模型、图片模型、RAG、TTS、视频或写法引擎偏好。
