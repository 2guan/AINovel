...
重大更新：系统全面升级为**多用户协作与租户安全隔离平台**，引入了高强度的用户认证（Authentication）、多租户物理隔离（Tenant Isolation）、细粒度角色权限（RBAC）以及管理员成员审批面板。同时重构了容器网络网关，实现了反向代理“零配置”自适应与高保真流式写作体验。
...
- **导演跟进页面与任务中心用户隔离：** 修复多用户环境下“导演跟进”页面和“任务中心”任务列表无隔离的数据泄漏问题——用户 B 之前能看到管理员 A 的所有运行任务及报错详情。现已在数据库查询层加入用户归属过滤，同时兼容历史存量任务（通过所属小说的用户归属自动映射），新建任务从创建时即记录归属用户，彻底实现任务数据的租户级安全隔离。
