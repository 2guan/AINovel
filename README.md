...
重大更新：系统全面升级为**多用户协作与租户安全隔离平台**，引入了高强度的用户认证（Authentication）、多租户物理隔离（Tenant Isolation）、细粒度角色权限（RBAC）以及管理员成员审批面板。同时重构了容器网络网关，实现了反向代理“零配置”自适应与高保真流式写作体验，同步准备 Windows 桌面版 `0.4.0`。

- **多用户认证与安全加密：** 新增 `User` 用户实体，基于纯 JS 实现的 `bcryptjs` 进行密码高强度哈希加密（完美兼容 ARM64 Docker 容器环境，解决原生 C++ bcrypt 构建崩溃的痛点），并配合 JWT 提供 7 天的免密登录状态。
- **租户级数据物理隔离 (Tenant Isolation)：** 小说（包括章节、角色、大纲、快照、中枢决策等子资源）、自定义世界观、自定义知识文档资产全部通过 `userId` 进行级联归属隔离。非管理员用户仅能读写自己名下的数据。
- **混合预设共享机制：** 题材基底、故事推进模式、反 AI 规则等系统预设（`userId === null`）全员共享可读，但仅限超级管理员（`admin`）能更新升级，保证创作者底座的一致性与内容安全。
- **待审核封锁与管理员面板 (User RBAC)：** 新注册用户默认身份为“待审核（pending）”，会被锁定在待审核主页且无法调用任何创作 API。管理员拥有唯一的“上帝视角”和“成员管理”看板，可一键审批通过、重置用户密码或手动创建成员账号。
- **管理员成员管理面板路由修复：** 移除了 `UserManagementPage.tsx` 中所有后台接口请求的冗余 `/api` 前缀，彻底解决 Axios 基于相对路径 `/api` 拼接导致的 API 报 404 重复路径错误（如 `/api/api/admin/users`），并确保成员管理加载顺滑。
- **反向代理零配置与同源网关：** 重构前端容器的 Nginx 配置文件 `ai-novel-web.conf`，将全部 `/api` 请求内部反向代理至后端的 `api` 容器，并将 Docker 容器构建的 `VITE_API_BASE_URL` 缺省值设为相对路径 `/api`。任何外网域名、SSL/TLS 证书反向代理只需映射 4002 端口即可，浏览器自动解析主机与协议，实现秒级零配置部署。
- **高保真流式写作体验优化：** Nginx 网关对 `/api` 路径新增了 SSE（Server-Sent Events）专属支持，包括禁用代理缓冲（`proxy_buffering off`）、关闭代理缓存（`proxy_cache off`）与启用块传输编码等，确保 AI 创意中枢大纲脑暴与正文生成等流式文字输出毫无卡顿，顺滑喷涌。
- **一键种子化历史回填 (Seeding & Migration)：** 全新的 `db:seed` 脚本在首次升级时自动创建超级管理员 `admin`（默认密码 `admin2026`），并将系统内已有的所有历史小说、世界、文档资产安全回填挂载至 `admin` 账号下，确保历史项目 100% 数据安全。
- **全面严格类型编译通过：** 前端 `client` 与后端 `server` 全量通过 TypeScript 严格类型检测编译（`0 错误`），修复了 jsonwebtoken 签名重载类型限制与 Express params 列表映射歧义。
- **AI 驾驶舱与任务中心孤立任务自动过滤 (Orphaned Tasks Filtering)：** 针对小说被删除后关联任务由于 `onDelete: SetNull` 级联删除策略转为 `novelId: null` 成为“孤立任务”的痛点，为自动导演驾驶舱跟进、任务管理适配器、可恢复任务扫描及手动恢复备选列表等核心接口，全面追加 `novelId: { not: null }` 严格物理存在过滤。彻底杜绝已删除小说的陈旧残留后台任务被误加载并因触发状态修复/检查点对账而反复抛出“小说不存在。”异常导致驾驶舱异常警告刷屏的故障。
- **导演跟进页面与任务中心用户隔离：** 修复多用户环境下"导演跟进"和"任务中心"任务列表的数据泄漏问题，每个用户现在只能看到自己名下小说的任务记录，历史存量任务通过所属小说的用户归属自动映射，新建任务从创建时即记录归属用户。
