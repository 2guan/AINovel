...
- **自适应数据库结构同步修复：** 针对 Linux 生产环境和 Docker 容器在全新部署或清空数据库重新构建时，由于 SQLite 历史迁移差异导致 `User` 等关键表在部分环境下未能成功初始化的故障，我们将容器启动时的数据库校验与结构同步机制升级为自适应模型同步（`db push`）。此举彻底消除了 schema 漂移与历史遗留迁移文件缺失的冲突，确保所有新建或重构的容器在首次启动时均能 100% 自动构建全套 100+ 张业务表，超级管理员账户与历史数据种子化回填过程能顺利且安全地注入，实现生产环境的秒级无痛一键拉起。
- **流式写作与创意中枢 401 认证修复：** 修复了多用户认证体系上线后，前端正文生成（`chapterSSE`）、正文修复（`repairSSE`）、创意中枢流式追问（`streamCreativeHubRun`）和智能对话（`AssistantChatPanel`）等核心 SSE 流式传输接口，由于直接使用原生 `fetch` 接口而遗漏注入 `Authorization` Bearer 身份令牌，导致在生成/对话时反复抛出 `401 Unauthorized` 鉴权失败拦截的问题。我们在流式连接握手时全面补全了 JWT 令牌安全注入与 401 自动令牌清理及重定向登录机制，确保高保真流式写作与中枢对话体验稳固运行。
- **章节队列底部遮挡滚动条缺陷修复：** 修复了在“章节执行”工作台中，左侧章节列表队列卡片（`ChapterExecutionQueueCard`）由于 Flex 容器尺寸高度溢出边界导致的最末端章节（如第 12 章等）在滚动时被容器边缘完全遮挡、无法完整滚至可视区的视觉与操作缺陷。我们将队列卡片修正为标准的弹性流式布局（`flex flex-col`），并为最底部追加了优雅的预留安全间距（`pb-16`），确保创作者在管理长篇多章节时能够极其顺滑、清晰地滚动并操作最后一个章节。
- **自动导演全局路由冲突与 404 修复：** 针对普通（非 admin）用户在“AI 自动导演开书”时点击“没有想法”会报错 `Request failed with status code 404 小说不存在` 的缺陷，我们深度分析了 Express 路由树的层级与前缀优先级，将特定前缀的自动导演路由 `/api/novels/director` 移至通配小说路由 `/api/novels` 之前，彻底消除了路由优先级覆盖带来的 404 越权拦截故障。同时，我们在自动导演路由内部全面补齐了精细化的路由级租户隔离校验（使用 `checkNovelAccess` 校验小说，设计并实现 `checkTaskAccess` 中间件校验任务所有权），确保多租户安全拦截无懈可击，普通用户开书与驾驶舱交互丝滑无阻。
- **书级 Framing 智能填写 404 冲突修复：** 修复了多用户隔离后，普通用户在小说基础信息页编辑“书级 framing”时，点击“AI 帮我填写”按钮报错 `Request failed with status code 404 小说不存在` 的问题。该故障的深层原因与自动导演开书报错一致——后台的 `/api/novels/framing/suggest` 静态请求被通用小说的 `/:id` 路由通配符拦截，导致 `"framing"` 被误判为小说 ID。我们在多租户鉴权中间件 `checkNovelAccess` 中引入了精确的静态保留路径豁免名单，安全屏蔽了包括 `director`、`framing` 和 `resource-recommendation` 等保留关键字，彻底消除了此类 Wildcard 路由参数名冲突，保障了整条新书规划与辅助创作流程的丝滑顺畅。
- **安全策略加固与默认凭证提示移除：** 移除了登录页面上显式展示的默认管理员账号密码（`admin / admin2026`）提示信息，降低了系统在公开部署或生产环境下的潜在越权与安全风险。
- **自主修改密码功能支持：** 在系统导航栏右上角用户名显示处，集成并上线了自助“修改密码”功能，用户可直接在工作台修改登录密码。系统会自动验证旧密码与新密码强度，修改成功后引导重新登录，完美对齐了后端多用户认证安全规范。
- **小说卡片创建者显示支持：** 针对多用户协作与审核体系上线后的协作场景，在小说列表卡片的右上角新增了优雅的“作者 (创建者)”标签，支持直观展示当前小说的创建者用户名，并在后端查询与前后端传输链路中全面补齐了租户归属数据透传，保障多用户管理体验清晰明了。
- **手机移动端退出登录支持：** 针对手机移动端布局，在顶部 Header 的右上角新增了精美的“退出登录”小图标按钮，与系统整体视觉风格完美契合，移动端创作者现在也可以在手机端顺畅、便捷地登出并切换账号。
