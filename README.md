...
SQLite 前后端版本支持多人共用同一套工作台：管理员可以审核成员、管理账号和查看所有小说；作家进入后只看到自己的小说、素材、知识库和项目资产。注册账号会先停留在等待审核页，避免未审核用户进入创作功能。模型配置也支持个人覆盖：默认继承管理员配置，用户需要时可以保存自己的模型密钥、当前模型选择和模型路由。

- 新增登录、注册、等待审核、退出登录和账号设置入口，用户可以在右上角统一修改显示名称和密码；初始管理员为 `admin / admin2026`。
- 手机端全局顶部和更多面板补齐账号入口，用户可以查看当前账号、打开账号设置、修改显示名称和密码，也可以直接退出登录。
- 系统菜单新增成员管理，管理员可以创建用户、审核注册用户、调整角色、停用账号、重置密码和删除非初始管理员账号。
- 提示词管理和成员管理只对管理员开放，普通作家不会进入这些后台管理功能。
- 小说列表按角色显示内容：管理员可查看全部小说并看到作者信息，作家只看到自己的小说。
- 从小说列表删除小说时，会同步移除这本书对应的任务中心记录和自动导演推进记录，避免已删除项目继续出现在恢复入口或导演跟进中心。
- 小说、世界样本、知识库文档、短剧项目、漫画项目、任务中心、恢复入口、导演跟进中心和主要创作资产会按登录用户隔离，减少团队共用时的内容串线。
- 模型密钥、模型路由、当前模型选择、图像模型、RAG 参数、写法引擎参数和自动导演偏好支持个人保存；没有个人配置时自动使用管理员默认配置，后台图像、写法、RAG、拆书、章节流水线和自动导演任务也会按任务所属用户读取这些配置。
- 作家确认自动导演开书方案后，新小说会创建到该作家账号下，后台规划也会继续按这本小说的所属账号推进，避免小说刚创建就因为用户隔离显示“小说不存在”。
- 后台队列会按任务所属账号执行图像生成、写法提取、拆书、章节流水线、章节资产同步、兑现账本同步、漫画事实抽取、恢复任务、短剧批量任务和漫画批量任务，减少多人共用时后台任务误用其他账号配置或看不到所属内容的情况。
- 创作中枢历史、拆书小节、写法引擎从拆书生成写法等子内容会先确认父资源属于当前用户，避免通过直接访问子内容编号看到其他用户内容。
- 小说列表新增公开阅读入口，可以在新页面打开精致阅读器；阅读页会加载本小说已生成正文的章节，支持桌面目录、手机章节切换、手机分页阅读、字号调节、夜间阅读和前后翻章，知道链接的游客也可以直接阅读。
- Docker 配置支持用 `docker compose up` 启动 SQLite API 服务和 Web 前端，默认在本机暴露 API `4001`、Web `4002`，也可以通过根目录 `.env` 调整端口；Web 镜像会带着当前应用版本完成构建。
- Docker 持久数据默认集中放在项目目录 `./AINovelData/` 下，SQLite 数据库和生成图片位于 `storage` 子目录，可选 Qdrant 数据位于 `qdrant` 子目录，方便服务器备份和迁移。
- Docker API 镜像补齐 Prisma 运行所需的 OpenSSL；使用 `./AINovelData/` 目录挂载时，给容器用户写权限后即可正常打开 SQLite 数据库。
- Docker 或反向代理访问时，登录页会先用公开健康检查确认服务可用，并兼容 API 地址末尾带斜杠的配置。
- SQLite Docker 部署会补齐提示词槽位覆盖和小说事实账本所需的数据表，减少写章上下文读取和提示词覆盖读取时的表缺失日志。
- 新建小说和创作中枢的默认目标章数更轻量，自动导演可以接受 1 章、3 章、5 章这类短篇目标；节奏 / 拆章和后续自动执行都会按目标章数收束范围，不会把 3 章故事套进 12 章、长篇六段式节奏或默认第 1-10 章执行范围。
- 书级 framing、封面、世界观、写法推荐、卷章规划和章节上下文里的“前期兑现”统一按全书前 30% 理解，短篇会按比例折算，不再要求必须写到固定章节号。
- 导航菜单隐藏“新手上路”、短剧工作台和漫画工作台入口，让工作台菜单更聚焦当前常用创作功能。
- 桌面版本号更新为 `0.3.20-MU`，用于区分多用户版本。
