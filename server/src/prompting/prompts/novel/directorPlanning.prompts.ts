...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "5. coreConflict 必须写清真正能支撑目标篇幅的主要矛盾；短篇可以聚焦单个强冲突，长篇才需要长期连锁矛盾。",
      ...
      "12. targetChapterCount 必须是合理的整书目标体量，与题材密度和推进方式相匹配；允许 1 章、3 章、5 章这类短篇，不要默认拉成长篇。",
      "",
      "【差异化要求】",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "",
      "【规划原则】",
      "1. 整体结构必须服从目标章节总数：1-5 章短篇可以只用一个紧凑 arc，长篇再展开多个阶段 arc。",
      "2. bookPlan 负责整书级承诺、主线、阶段推进与总节奏控制。",
      "3. arcs 必须体现明确阶段功能，不能只是把章节机械分组。",
      "4. 每个 arc 都要说明自己为什么单独存在，它负责哪一段阶段性承诺、冲突升级或关系变化；短篇不要为了凑阶段硬拆 arc。",
...
> = {
  ...
  structuredOutputHint: {
    ...
    example: {
      ...
      chapter30Payoff: "示例：全书前 30% 位置的阶段承诺兑现。",
  ...
  render: (input, context) => [
    new SystemMessage([
      "你是网文总导演，负责把已确认的书级方向收束成一本书的 Book Contract。",
      "服务对象是不懂写作流程的新手用户。",
      ...
      "4. chapter3Payoff、chapter10Payoff、chapter30Payoff 必须体现明确的篇幅兑现节奏。短篇要按目标章节数折算：chapter3Payoff 可对应开篇抓手，chapter10Payoff 可对应中点回报，chapter30Payoff 对应全书前 30% 的关键兑现，不要机械要求真的写到固定章节号。",
      ...
      "2. 它不是宣传文案，而是创作约束文件。",
      "3. 它必须同时服务前期抓力、中段续航和整本完成度；短篇优先保证紧凑闭环，长篇再强调长期连载稳定性。",
      "4. 对新手用户要清晰，不能写成模糊抽象的文学评论。",
      "",
      "【质量要求】",
      "1. chapter3Payoff 应偏向开书抓手与快速兑现。",
      "2. chapter10Payoff 应体现中前段建立后的第一阶段回报或局面变化。",
      "3. chapter30Payoff 应体现全书前 30% 的阶段性兑现；短篇可对应前段完成的关键抓手或第一个明确转向。",
